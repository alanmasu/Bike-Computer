...
{
         TIMER_A_CLOCKSOURCE_EXTERNAL_TXCLK,               //frequency: 32.768 kHz
         TIMER_A_CLOCKSOURCE_DIVIDER_1,         //new frequency: 512 Hz
         TIMER_A_TAIE_INTERRUPT_DISABLE,                                    //timer counts 512 rising edges of the clock: 1s (qualsiasi sia questo valore, e' importante contare sempre fino a 1s)
         TIMER_A_DO_CLEAR
};
...
int main(void)
{
    ...
    MAP_GPIO_setAsPeripheralModuleFunctionInputPin(GPIO_PORT_P4, GPIO_PIN2, GPIO_SECONDARY_MODULE_FUNCTION);
  ...
  /* Configuring Capture Mode */
   // MAP_Timer_A_initCapture(TIMER_A0_BASE, &captureModeConfig);
    ...
    while(1){
        if(isrFlag){
            printf("Register value: %d", capturedValue);
            isrFlag = false;
        }
...
{
     //   capturedValue = MAP_Timer_A_getCaptureCompareCount(TIMER_A0_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_1);
    capturedValue = Timer_A_getCounterValue(TIMER_A2_BASE);
